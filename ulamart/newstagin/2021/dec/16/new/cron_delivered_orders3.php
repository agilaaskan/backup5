...
try  // try starts
{
	...
	// 	while($row = $result_select_orders->fetch_assoc()) {
			...
			if(strtolower($res_dtdc['strAction']) == 'delivered') { 
				...
				if(strlen($delivered_name)>4 && strpos(strtolower($delivered_name), 'sign') === false) {
	...
	$headers .= "Return-Path: <info@ulamart.com> \n";
...
{		
		...
		$ptn = "/^0/";
		$sqls1 = "SELECT sales_order_address.telephone,sales_order_address.firstname,sales_order_address.lastname,sales_order_address.email FROM `sales_order_grid` INNER JOIN sales_order_address ON sales_order_grid.entity_id = sales_order_address.parent_id WHERE sales_order_grid.increment_id = '$orderid' LIMIT 1";
		...
		CURLOPT_POSTFIELDS =>'{
		  ...
		  "traits": {
			  ...
			  "DeliveredName": "'.$order_result['delivered_name'].'"
			  }
			}',
