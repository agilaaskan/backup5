...
else
{
    ...
    if ($result_partial_orders_custom->num_rows > 0)
    {
        ...
        while ($row = $result_partial_orders_custom->fetch_assoc())
        {
            ...
            $increement_id = $row['increement_id'];
            ...
            for ($k = 0;$k < count($weightList);$k++)
            {
                $totalboxwt = $totalboxwt + floatval($weightList[$k]->weight);
                    ...
                    $ptn = "/^0/";
                    $sqls1 = "SELECT sales_order_address.telephone,sales_order_address.firstname,sales_order_address.lastname,sales_order_address.email FROM `sales_order_grid` INNER JOIN sales_order_address ON sales_order_grid.entity_id = sales_order_address.parent_id WHERE sales_order_grid.increment_id = '$increement_id' LIMIT 1";
