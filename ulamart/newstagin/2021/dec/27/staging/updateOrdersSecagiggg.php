...
if ($page == 1)
{
    ...
    SET pictures= '$pictures' , updated_on ='$createdon'
...
{
    $orderid = '000038923';
    $consinment = '22117022700';
    $courier_type = 'F';
    $createdon = '2021-12-16 15:42:37';
    $partial_order_id = 7049;

    $update_record = "UPDATE ps_partial_orders_custom SET consignment_no= '$consinment' , courier_type = '$courier_type', mail_sent = 1, status = 'completed', updated_on ='$createdon'
    WHERE id = '$partial_order_id'";
    if ($conn->query($update_record) === true)
    {
        $MSG = 'Data Updated Successfully into MySQL Database';
    ...
    if ($result_partial_orders_custom->num_rows > 0)
    {
        ...
        while ($row = $result_partial_orders_custom->fetch_assoc())
        {
            ...
            $weightList = json_decode(json_encode($attachment), true);

            $totalboxwt = 0;
            for ($k = 0;$k < count(array($weightList));$k++)
            {
                $totalboxwt = $totalboxwt + floatval($weightList[$k]->weight);
                    ...
                    $ptn = "/^0/";
                    $sqls1 = "SELECT sales_order_address.telephone,sales_order_address.firstname,sales_order_address.lastname,sales_order_address.email FROM `sales_order_grid` INNER JOIN sales_order_address ON sales_order_grid.entity_id = sales_order_address.parent_id WHERE sales_order_grid.increment_id = '$order_id' LIMIT 1";
                    ...
                    if (send_email($customer_email, $attachment, $message, $order_id, $courier_type, $telephone, $checkname, $checkem, $tracking ,$total_bx ,$totalboxwt))
                    {
...
function send_email($to, $attachment, $message, $order_number, $courier_type, $telephone, $checkname, $checkem, $tracking ,$total_bx ,$totalboxwt)
{
    ...
    // try {
        ...
        for($j =0; $j < count($files);  $j++)
        {
            ...
            if($j == 0) {
               $whtimg = "https://dev.ulamart.com/API/ulatools/Magento/".$images1;
            }
        }
