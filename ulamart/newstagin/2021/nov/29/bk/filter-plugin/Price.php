...
class Price extends AbstractFilter
{
	...
	public function apply(\Magento\Framework\App\RequestInterface $request)
	{
		...
		$currencys = $currencysymbol->getStore()->getCurrentCurrencyCode();
		list($from, $to) = $filter;
		...
		if ($currencys == "USD") {
			$convertrupee = $this->_helperFunction->isConvert();
			$url = $_SERVER['REQUEST_URI'];
			if (strpos($url, "?price=")!==false){
				if(($from >= $convertrupee ) || ($from == 9.99)) {
					$from = $from / $convertrupee;
						$from = round($from);
							if ($from <= 0) {
								$from = 1;
							}                
